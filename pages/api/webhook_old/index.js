...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      // Store image URLs and other relevant data
      if (webhookData.image_url) {
        item.image_url = webhookData.image_url;
        item.image_url_png = webhookData.image_url_png;
        item.image_url_jpg = webhookData.image_url_jpg;
      }

      // For collections, store additional information
      if (webhookData.template_set) {
        item.template_set = webhookData.template_set;
        item.image_urls = webhookData.image_urls || {};
        item.images = webhookData.images || [];
        item.total_images = webhookData.images?.length || 0;
        item.zip_url = webhookData.zip_url;
      }
